...
export function buildFujianTaiwanRiceNoodleResearchFromFixedInputs() {
  const report = buildFujianTaiwanRiceNoodleResearchReport({
    assessment: readJson('tools/data/fujian-taiwan-rice-noodle-research.v1.json'),
  ...
  const reportErrors = validateFujianTaiwanRiceNoodleResearchReport(report);
  if (reportErrors.length) throw new Error(`Fujian-Taiwan research report validation failed:\n${reportErrors.map(error => `- ${error}`).join('\n')}`);
  return { report, artifacts: buildFujianTaiwanRiceNoodleResearchArtifacts(report) };
}

function main() {
  const args = process.argv.slice(2);
  if (args.length > 1 || (args.length === 1 && !['--write', '--check'].includes(args[0]))) {
    console.error('Usage: node tools/build-fujian-taiwan-rice-noodle-research.mjs [--write|--check]');
    process.exitCode = 2;
    return;
  }
  let result;
  try {
    result = buildFujianTaiwanRiceNoodleResearchFromFixedInputs();
  } catch (error) {
  ...
  const mode = args[0];
  if (!mode) {
    console.log(formatFujianTaiwanRiceNoodleResearchSummary(result.report));
    return;
  }
  if (mode === '--write') {
