...
test('health reports exact validated planner asset versions and catalog counts', async () => {
  ...
  assert.equal(result.body.templateCatalogVersion, 'templates-v2-20260727-r7');
  assert.equal(result.body.ingredientTaxonomyVersion, 'taxonomy-v1-20260727-r6');
...
test('deployment documentation tracks the current draft planner asset baseline', () => {
  ...
  assert.match(deployment, /templates-v2-20260727-r7/);
  assert.match(deployment, /taxonomy-v1-20260727-r6/);
  assert.match(deployment, /ratio-rules-v1-20260727-r4/);
  assert.match(deployment, /10 个 active templates，6 个 planned templates/);
  assert.match(deployment, /100\/100/);
