...
test('health reports exact validated planner asset versions and catalog counts', async () => {
  ...
  assert.equal(result.body.ingredientTaxonomyVersion, 'taxonomy-v1-20260802-r12');
  assert.equal(result.body.ratioRulesVersion, 'ratio-rules-v1-20260802-r14');
  ...
  assert.equal(result.body.sourceRuntimeCatalogBlocked, 1);
  assert.equal(result.body.sourceRuntimeCatalogFormalActive, 0);
  assert.equal(result.body.sourceRuntimeCatalogKitchenObserved, 0);
  assert.equal(result.body.sourceCoverageMatrix, 'ok');
  assert.equal(result.body.sourceCoverageMatrixVersion, 'source-backed-coverage-matrix-v1-20260813-m1');
  assert.equal(result.body.sourceCoverageMatrixEntries, 923);
  assert.equal(result.body.sourceCoverageMatrixP0, 138);
  assert.equal(result.body.sourceCoverageMatrixP1, 738);
...
test('deployment documentation tracks the current draft planner asset baseline', () => {
  ...
  assert.match(deployment, /taxonomy-v1-20260802-r12/);
  assert.match(deployment, /ratio-rules-v1-20260802-r14/);
