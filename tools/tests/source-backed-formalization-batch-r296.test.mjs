...
test('r296 marks the third five exact source contracts as bounded but non-executable', () => {
  ...
  assert.ok(evidence.counts.source_bounded_non_executable >= 15);
});

test('r296 retains the source appliance boundary and formal blockers', () => {
  ...
  assert.ok(review.counts.ratio_dsl.source_bounded >= 15);
