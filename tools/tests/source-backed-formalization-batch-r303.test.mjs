...
test('r303 marks the tenth five exact source contracts as bounded but non-executable', () => {
  ...
  assert.ok(evidence.counts.source_bounded_non_executable >= 50);
});

test('r303 retains public-institution appliance and staged boundaries', () => {
  ...
  assert.ok(review.counts.ratio_dsl.source_bounded >= 50);
