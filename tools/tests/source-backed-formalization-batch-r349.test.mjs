...
const targets = [
  ['source-tiger-chinese-sticky-rice', 'tiger-chinese-sticky-rice'],
  ['source-panasonic-claypot-style-chicken-rice', 'panasonic-claypot-style-chicken-rice'],
  ['source-tatung-tomato-pumpkin-rice', 'tatung-tomato-pumpkin-rice'],
];

const variants = () => catalog.families.flatMap(family => family.variants || []);

test('r349 adds three source-bounded calibration previews without changing the formal 72', () => {
  for (const [variantId, sourceId] of targets) {
    ...
    const journey = journeys.journeys.find(row => row.id === `RM-35${5 + targets.findIndex(item => item[0] === variantId)}-${variantId}`);
...
test('r349 keeps all three variants calibration-only and out of production Planner', () => {
