...
test('r302 marks the ninth five exact source contracts as bounded but non-executable', () => {
  ...
  assert.ok(evidence.counts.source_bounded_non_executable >= 45);
});

test('r302 retains public, pan and congee boundaries while keeping Planner blockers', () => {
  ...
  assert.ok(review.counts.ratio_dsl.source_bounded >= 45);
