...
const targetId = 'tefal-pilaf-with-lamb-r200302';
...
test('r235 preserves the source cut, staged Cook4me process, and non-executable boundary', () => {
  const catalog = JSON.parse(readFileSync(catalogPath, 'utf8'));
  const recipe = catalog.recipes.find(item => item.recipe_id === targetId);
  assert.ok(recipe, targetId);
  assert.equal(recipe.fixed_batch.servings, 4);
  assert.equal(recipe.fixed_batch.ingredients.find(item => item.name === '羊腿肉')?.amount.value, 450);
  assert.equal(recipe.liquid_contract.amount.value, 800);
  assert.equal(recipe.time_contract.total_minutes, 80);
  assert.equal(recipe.cooker_adaptation.status, 'source_limited');
  assert.match(recipe.evidence_notes, /羊腿肉|Cook4me|不改写器具/u);
  assert.match(recipe.cooker_adaptation.notes, /Cook4me|不推导普通电饭煲/u);
});
