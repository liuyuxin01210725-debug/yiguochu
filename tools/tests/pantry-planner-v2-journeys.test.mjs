...
test('corpus maps spec journeys 1-44 exactly once', () => {
  ...
  assert.deepEqual(
    ...
    { taxonomy_shape:8, recommend:3, pantry_coverage:8, decision:5, intent_swap:7, model_boundary:7, version_legacy:6 },
  );
...
test('CLI executes the gate and emits its stable summary line', () => {
  ...
  assert.match(result.stdout, /(?:^|\n)44\/44 planner v2 journeys passed(?:\n|$)/);
});
