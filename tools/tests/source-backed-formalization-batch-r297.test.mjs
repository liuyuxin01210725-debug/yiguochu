...
test('r297 marks the fourth five exact source contracts as bounded but non-executable', () => {
  ...
  assert.ok(evidence.counts.source_bounded_non_executable >= 20);
});

test('r297 preserves claypot and manufacturer boundaries while keeping Planner blockers', () => {
  ...
  assert.ok(review.counts.ratio_dsl.source_bounded >= 20);
