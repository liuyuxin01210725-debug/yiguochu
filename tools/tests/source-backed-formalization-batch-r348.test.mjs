...
const targets = [
  ['source-zojirushi-okayama-ebimeshi', 'zojirushi-okayama-ebimeshi-el-ns23'],
  ['source-cookpot-beef-wild-mushroom-rice', 'cookpot-beef-wild-mushroom-rice'],
  ['source-tatung-hainan-chicken-rice', 'tatung-hainan-chicken-rice'],
  ['source-tatung-pork-jowl-sesame-rice', 'tatung-pork-jowl-sesame-rice'],
];

const variants = () => catalog.families.flatMap(family => family.variants || []);

test('r348 adds four source-bounded calibration previews without changing the formal 72', () => {
  for (const [variantId, sourceId] of targets) {
    ...
    const journey = journeys.journeys.find(row => row.id === `RM-3${51 + targets.findIndex(item => item[0] === variantId)}-${variantId}`);
...
test('r348 keeps all four variants calibration-only and out of production Planner', () => {
