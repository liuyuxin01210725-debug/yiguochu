...
test('r299 marks the sixth five exact source contracts as bounded but non-executable', () => {
  ...
  assert.ok(evidence.counts.source_bounded_non_executable >= 30);
});

test('r299 preserves Tiger, Panasonic, claypot and steamed-rice boundaries', () => {
  ...
  assert.ok(review.counts.ratio_dsl.source_bounded >= 30);
