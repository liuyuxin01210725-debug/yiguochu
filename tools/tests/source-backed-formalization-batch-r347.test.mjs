...
const targets = [
  ['source-taiwan-tatung-cabbage-rice', 'taiwan-tatung-cabbage-rice'],
  ['source-zojirushi-pork-vegetable-rice', 'zojirushi-pork-vegetable-rice-el-ns23'],
  ['source-panasonic-nf-pc400-takikomi-rice', 'panasonic-nf-pc400-takikomi-rice'],
  ['source-panasonic-khao-man-gai', 'panasonic-khao-man-gai-nf-ac1000'],
];

const variants = () => catalog.families.flatMap(family => family.variants || []);

test('r347 adds four source-bounded calibration previews without changing the formal 72', () => {
  for (const [variantId, sourceId] of targets) {
    ...
    const journey = journeys.journeys.find(row => row.id === `RM-3${47 + targets.findIndex(item => item[0] === variantId)}-${variantId}`);
...
test('r347 keeps all four variants calibration-only and out of production Planner', () => {
