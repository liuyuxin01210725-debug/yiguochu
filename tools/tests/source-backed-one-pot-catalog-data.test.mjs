...
test('r57 collection batch records first-party vendor, institutional and regional one-pot candidates without promoting research', () => {
  ...
  assert.equal(catalog.catalog_version, 'source-backed-one-pot-v1-20260808-national-r67');
  assert.equal(catalog.recipes.length, 618);
  const expected = [
...
test('keeps research entries non-public after signed contracts are admitted', () => {
  ...
  assert.deepEqual(statusTotals, {
    discovered: 6,
    executable: 12,
    identity_verified: 52,
    recipe_fact_checked: 548,
  });
