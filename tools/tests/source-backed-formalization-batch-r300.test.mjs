...
test('r300 marks the seventh five exact source contracts as bounded but non-executable', () => {
  ...
  assert.ok(evidence.counts.source_bounded_non_executable >= 35);
});

test('r300 retains the manufacturer and pan boundaries while keeping Planner blockers', () => {
  ...
  assert.ok(review.counts.ratio_dsl.source_bounded >= 35);
