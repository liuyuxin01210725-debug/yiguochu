...
import { buildLingnanHkMacaoOnePotResearchArtifacts } from './lib/lingnan-hk-macao-one-pot-research-renderer.mjs';

const ROOT = fileURLToPath(new URL('..', import.meta.url));
const readJson = relativePath => JSON.parse(fs.readFileSync(path.join(ROOT, relativePath), 'utf8'));

export function buildLingnanHkMacaoOnePotResearchFromFixedInputs() {
  const inputs = {
    assessment: readJson('tools/data/lingnan-hk-macao-one-pot-research.v1.json'),
  ...
  const sourceErrors = validateLingnanHkMacaoOnePotResearch(inputs);
  if (sourceErrors.length) throw new Error(`Lingnan Hong Kong Macao research input validation failed:\n${sourceErrors.map(error => `- ${error}`).join('\n')}`);
  const report = buildLingnanHkMacaoOnePotResearchReport(inputs);
  const reportErrors = validateLingnanHkMacaoOnePotResearchReport(report);
  if (reportErrors.length) throw new Error(`Lingnan Hong Kong Macao research report validation failed:\n${reportErrors.map(error => `- ${error}`).join('\n')}`);
  return { report, artifacts: buildLingnanHkMacaoOnePotResearchArtifacts(report) };
}

function main() {
  const [mode] = process.argv.slice(2);
  if (!['--write', '--check'].includes(mode) || process.argv.length !== 3) {
    console.error('Usage: node tools/build-lingnan-hk-macao-one-pot-research.mjs --write|--check');
    process.exitCode = 2;
    return;
  }
  let result;
  try {
    result = buildLingnanHkMacaoOnePotResearchFromFixedInputs();
  } catch (error) {
  ...
  if (mode === '--write') {
    ...
    console.log(formatLingnanHkMacaoOnePotResearchSummary(result.report));
    return;
  }
  const stale = [];
  for (const [relativePath, content] of result.artifacts) {
