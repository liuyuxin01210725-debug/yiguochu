...
function isHttps(value) {
  return presentString(value) && /^https:\/\//.test(value);
}
...
export function validateRiceCookerSourceEvidence(ledger) {
  ...
  ledger.entries.forEach((entry, index) => {
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(entry.retrieved_at || '')) errors.push(`${path}.retrieved_at must be YYYY-MM-DD`);
    ...
    if (!isObject(entry.rights)) {
      ...
      if (!Array.isArray(entry.rights.allowed_use) || entry.rights.allowed_use.length === 0) errors.push(`${path}.rights.allowed_use is required`);
      if (!Array.isArray(entry.rights.forbidden_use) || entry.rights.forbidden_use.length === 0) errors.push(`${path}.rights.forbidden_use is required`);
      if (entry.rights.license_url !== null && !isHttps(entry.rights.license_url)) errors.push(`${path}.rights.license_url must be HTTPS or null`);
      if (entry.rights.contribution_permission_url !== null && !isHttps(entry.rights.contribution_permission_url)) {
    ...
    if (!isObject(entry.quantities)) {
      ...
      if (!Array.isArray(entry.quantities.protein_items)) errors.push(`${path}.quantities.protein_items must be an array`);
      else entry.quantities.protein_items.forEach((item, itemIndex) => validateQuantityItem(item, `${path}.quantities.protein_items[${itemIndex}]`, errors));
      if (!Array.isArray(entry.quantities.vegetable_items)) errors.push(`${path}.quantities.vegetable_items must be an array`);
      else entry.quantities.vegetable_items.forEach((item, itemIndex) => validateQuantityItem(item, `${path}.quantities.vegetable_items[${itemIndex}]`, errors));
...
export function assertRiceCookerSourceEvidence(ledger) {
  ...
  if (errors.length > 0) throw new Error(`invalid rice-cooker source evidence:\n${errors.join('\n')}`);
  return ledger;
}
