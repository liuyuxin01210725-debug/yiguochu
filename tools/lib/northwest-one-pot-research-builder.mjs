...
const EXPECTED_PROVINCES = ['CN-SN', 'CN-GS', 'CN-NX', 'CN-XJ'];
...
function deriveCompletion(report) {
  ...
  const audited = [...production, ...leads];
  ...
  if (audited.some(row => Object.entries(asObject(row.claims)).some(([claimId, claim]) => /ratio|time_vessel|time_safety/.test(claimId) && claim?.verdict !== 'supported'))) blockers.push('ratio_dsl_unresolved');
  if (audited.some(row => Object.entries(asObject(row.claims)).some(([claimId, claim]) => /adaptation|equivalence|appliance/.test(claimId) && claim?.verdict !== 'supported'))) blockers.push('household_vessel_adaptation_unresolved');
  ...
  if (journeys.length !== 16 || reviewed !== 16) blockers.push('human_journey_review_incomplete');
  return {
...
export function buildNorthwestOnePotResearchReport({
  assessment, recipeLibrary, regionalResearch, regionalAtlas, regionalMappings,
} = {}) {
  const errors = validateNorthwestOnePotResearch({ assessment, recipeLibrary, regionalResearch, regionalAtlas, regionalMappings });
  ...
  const provinceCoverage = source.province_codes.map(provinceCode => {
    ...
    return {
      ...
      production_recipe_ids: production.filter(row => row.province_code === provinceCode).map(row => row.recipe_id).sort(),
      candidate_ids: candidates.filter(row => row.province_code === provinceCode).map(row => row.candidate_id).sort(),
...
export function validateNorthwestOnePotResearchReport(report) {
  ...
  if (report.region_overview?.region_id !== 'northwest') errors.push('region_overview must be Northwest');
  if (JSON.stringify(report.region_overview?.province_codes) !== JSON.stringify(EXPECTED_PROVINCES)) errors.push('region_overview province_codes must match the four fixed provinces');
  ...
  if (asArray(coverage.get('CN-GS')?.production_recipe_ids).length !== 0 || asArray(coverage.get('CN-NX')?.production_recipe_ids).length !== 0) errors.push('Gansu and Ningxia must retain zero production audits');
  if (JSON.stringify(asArray(coverage.get('CN-XJ')?.production_recipe_ids)) !== JSON.stringify(['xinjiang-lamb-pilaf', 'xinjiang-vegetable-pilaf'])) errors.push('Xinjiang must retain its two production audits');
  ...
  if (JSON.stringify(report.completion?.blockers) !== JSON.stringify(FIXED_COMPLETION_BLOCKERS)) errors.push('completion blockers must retain source, ratio, household adaptation, safety and human review gates');
  ...
  if (expectedCompletion.status !== 'research_in_progress' || JSON.stringify(expectedCompletion.blockers) !== JSON.stringify(FIXED_COMPLETION_BLOCKERS)) errors.push('audited rows no longer support the fixed Northwest research gate');
...
export function formatNorthwestOnePotResearchSummary(report) {
  const summary = asObject(report?.summary);
  const status = report?.completion?.status === 'research_in_progress' ? 'research in progress' : 'research ok';
  return `${summary.production_audit_count ?? 0} Northwest production audits · ${summary.candidate_audit_count ?? 0} candidates · ${summary.concrete_research_lead_count ?? 0} research leads · ${summary.household_journey_count ?? 0} journeys · Northwest ${status}`;
}
