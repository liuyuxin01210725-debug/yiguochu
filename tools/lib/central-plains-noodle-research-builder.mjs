import { validateCentralPlainsNoodleResearch } from './central-plains-noodle-research-validator.mjs';
...
function shapeMatrix(productionAudits, candidateAudits, leads) {
  const index = new Map();
  const add = (shape, subjectType, subjectId) => {
    if (!hasText(shape)) return;
    if (!index.has(shape)) index.set(shape, {
      production_recipe_ids: [], candidate_ids: [], research_lead_ids: [], source_fields: [],
    });
    const entry = index.get(shape);
    if (subjectType === 'production') entry.production_recipe_ids.push(subjectId);
    else if (subjectType === 'candidate') entry.candidate_ids.push(subjectId);
    else entry.research_lead_ids.push(subjectId);
  ...
  for (const lead of leads) {
    for (const shape of asArray(lead?.ingredient_shapes)) add(shape, 'lead', lead.lead_id);
  }
  return [...index.entries()].map(([shape, entry]) => ({
    ...
    research_lead_ids: [...new Set(entry.research_lead_ids)].sort(),
    source_fields: [...new Set(entry.source_fields)].sort(),
  })).sort((a, b) => a.shape.localeCompare(b.shape));
}

function deriveCompletion(productionAudits, candidateAudits, boundaries, safetyBoundaries, journeys) {
  const production = productionAudits[0];
  const candidates = new Map(candidateAudits.map(row => [row.candidate_id, row]));
  const blockers = [];
  if (production?.claims?.henan_specific_origin?.verdict !== 'supported'
    || candidates.get('henan-bean-pork-steamed-noodles')?.claims?.henan_exclusive_identity?.verdict !== 'supported') {
    blockers.push('province_specific_identity_gaps');
  }
  if (candidates.get('henan-celery-pork-steamed-noodles')?.claims?.fixed_regional_core?.verdict !== 'supported'
    || candidates.get('henan-cabbage-mushroom-steamed-noodles')?.claims?.fixed_regional_core?.verdict !== 'supported') {
    blockers.push('candidate_core_evidence_gaps');
  }
  const vessel = candidates.get('henan-home-one-pot-steamed-braised-noodles');
  const vesselBoundary = boundaries.find(row => row?.boundary_id === 'single-pot-adaptation-not-traditional-equivalence');
  if (vessel?.claims?.vessel_process_equivalence?.verdict !== 'supported' || vesselBoundary?.evidence_status !== 'checked') {
    blockers.push('vessel_process_equivalence_incomplete');
  }
  blockers.push('ratio_moisture_evidence_incomplete');
  if (safetyBoundaries.length === 0 || safetyBoundaries.some(row => row?.evidence_status !== 'machine_ready')) {
    blockers.push('safety_endpoint_incomplete');
  }
...
export function buildCentralPlainsNoodleResearchReport({
  assessment,
  recipeLibrary,
  regionalResearch,
  regionalAtlas,
  ...
  const sourceErrors = validateCentralPlainsNoodleResearch({
    assessment, recipeLibrary, regionalResearch, regionalAtlas, regionalMappings,
  });
  if (sourceErrors.length) throw new Error(`Invalid Central Plains noodle research:\n${sourceErrors.join('\n')}`);
  ...
  const region = asObject(asArray(regionalAtlas?.regions).find(row => row?.region_id === 'central_plains'));
  const province = asObject(asArray(regionalAtlas?.province_nodes).find(row => row?.atlas_code === 'CN-HA'));
  ...
  const productionAudits = asArray(source.production_recipe_audits).map(audit => {
  ...
  const candidateAudits = asArray(source.candidate_audits).map(audit => {
  ...
  const sources = asArray(source.source_refs).map(clone);
  const leads = asArray(source.concrete_research_leads).map(clone);
  const families = asArray(source.family_model).map(clone);
  const boundaries = asArray(source.adaptation_boundaries).map(clone);
  const safety = asArray(source.safety_boundaries).map(clone);
  const journeys = asArray(source.journey_cases).map(clone);
  const completion = deriveCompletion(productionAudits, candidateAudits, boundaries, safety, journeys);
  ...
  return {
    ...
    ingredient_shape_matrix: shapeMatrix(productionAudits, candidateAudits, leads),
...
function expectedCompletion(report) {
  return deriveCompletion(
    ...
    asArray(report?.adaptation_boundaries),
    asArray(report?.safety_boundaries),
    asArray(report?.household_journeys),
  );
}

export function validateCentralPlainsNoodleResearchReport(report) {
  ...
  if (sources.length !== 8) errors.push('source_evidence must contain exactly 8 items');
  if (journeys.length !== 12) errors.push('household_journeys must contain exactly 12 items');
  if (report.region_overview?.region_id !== 'central_plains') errors.push('region_overview must be central_plains');
  if (JSON.stringify(report.region_overview?.province_codes) !== JSON.stringify(['CN-HA'])) errors.push('region_overview province_codes must contain CN-HA');
...
export function formatCentralPlainsNoodleResearchSummary(report) {
  const summary = asObject(report?.summary);
  return `${summary.production_audit_count ?? 0} Central Plains production audit · ${summary.candidate_audit_count ?? 0} candidates · ${summary.household_journey_count ?? 0} journeys · Central Plains research ok`;
}
