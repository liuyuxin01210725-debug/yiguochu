...
export function buildYunnanGuizhouRiceResearchFromFixedInputs() {
  const inputs = {
    assessment: readJson('tools/data/yunnan-guizhou-rice-research.v1.json'),
  ...
  const sourceErrors = validateYunnanGuizhouRiceResearch(inputs);
  if (sourceErrors.length) throw new Error(`Yunnan-Guizhou research input validation failed:\n${sourceErrors.map(error => `- ${error}`).join('\n')}`);
  const report = buildYunnanGuizhouRiceResearchReport(inputs);
  const reportErrors = validateYunnanGuizhouRiceResearchReport(report);
  if (reportErrors.length) throw new Error(`Yunnan-Guizhou research report validation failed:\n${reportErrors.map(error => `- ${error}`).join('\n')}`);
  return { report, artifacts: buildYunnanGuizhouRiceResearchArtifacts(report) };
}

function main() {
  const [mode] = process.argv.slice(2);
  if (!['--write', '--check'].includes(mode) || process.argv.length !== 3) {
    console.error('Usage: node tools/build-yunnan-guizhou-rice-research.mjs --write|--check');
    process.exitCode = 2;
    return;
  }
  let result;
  try {
    result = buildYunnanGuizhouRiceResearchFromFixedInputs();
  } catch (error) {
  ...
  if (mode === '--write') {
    ...
    console.log(formatYunnanGuizhouRiceResearchSummary(result.report));
    return;
  }
  const stale = [];
  for (const [relativePath, content] of result.artifacts) {
