...
## r96 搜集期第六十九批（2026-08-06）

本批按未闭合官方线索、厂商官方和地域空白并行核验。基于 r94 的 765 条目录（r95 无新增），去重后新增 **1 条**：`recipe_fact_checked`；版本 bump 为 `source-backed-one-pot-v1-20260806-national-r96`，当前总量 **766**：`executable=12`、`recipe_fact_checked=669`、`identity_verified=79`、`discovered=6`、`kitchen_observed=0`。本批没有修改 Worker、前端、Planner、模板或 DeepSeek，没有部署。

### 本批新增

- `rda-korean-ginseng-chicken-nutrition-rice` — `인삼 영양밥（韩国人参营养饭）`：韩国农村振兴厅官方直达页第71至76行记录大米3杯、鸡肉100g、人参2根、栗子4个、红枣6个、鸡高汤7杯、浸米约30分钟和煮饭流程；原文未指定电饭煲、固定成品份数、完整总时间或独立鸡肉熟制终点，且人参与“除人参外”步骤的投料时机边界保持原样，因此停在研究层。

### 另外两路结果与浏览器说明

- 厂商线：东芝、象印、松下和苏泊尔/美的入口本轮未形成新的可核验非重复条目；重复菜名、只有标题的动态索引和产品宣传不计入新增。
- 地域线：新疆抓饭、酉州/玉屏社饭、广式腊味煲仔饭等均已有目录；纯碳水、熟饭二次加工、无独立地域/流程事实的候选排除。
- 本轮按用户要求尝试使用 Codex 内置浏览器（非 Chrome）打开新增来源和官方入口，但动态官方页面在内置浏览器中超时，未把超时结果当作证据；新增条目的直接原文核验来自机构线报告中记录的官方直达页和定位。后续若该来源需要独立复核，仍应保持 `recipe_fact_checked`，不得越级。
...
- 只记录官方直达页实际明确的身份、食材、定量、液体和流程；不把韩国页面的煮饭字样改写成电饭煲适配。
- 缺少安全终点、固定份数或器具转换参数保持空缺；未晋升 `executable` 或 `kitchen_observed`。
- r96 测试、目录产物和校验门禁均已更新；未新增弱来源，不为数量降低证据门槛。
...
### 艾丽丝欧雅玛官方来源（3 条，全部 `recipe_fact_checked`）

- `iris-kpc-ma2-tai-meshi` — 鯛飯：KPC-MA2 官方页给出 2–3 人份、米 2 合、鲷鱼切片约 120g、水 320mL、自动菜单 31 和 75 分钟；保留电气压力锅的排气与程序边界，不与爱媛/广岛等其他鲷鱼饭版本拼接。
- `iris-pc-mb3-ikameshi` — いかめし：PC-MB3-H 官方页给出鱿鱼 2 杯、糯米 100g、水 400mL、压力调理 30 分钟、总调理 1 小时 15 分钟，并明确糯米只装约一半以避免胀裂；保留厂商器具版本，不替代北海道地方来源。
- `iris-kpc-rema3-takikomi-rice` — 炊き込みご飯：KPC-REMA3 官方页给出鸡胸、胡萝卜、牛蒡、香菇、蒟蒻、2 合米、2 合水位线、压力 10 分钟和总调理 55 分钟；标题是厂商通用菜名，别名只作检索，不包装成地域菜，也不外推普通电饭煲。

### 本批边界与排除

- 三条均有官方直达 HTML、显式 evidence tier、正文定位、署名和许可字段；全部停在 `recipe_fact_checked`，没有自动晋升 `executable`，没有厨房验证。
- 甘南合作市官方资料中的“蕨麻米饭”明确是大米和蕨麻分别煮熟后分装、加糖和酥油，属于甜味主食/文化记录，不符合当前“同锅或连续一锅炊煮”主线，本批不写入目录，仅保留为排除线索。
- Iris 的“いかめし”与目录中日本农林水产省北海道版本是不同来源、不同器具合同；不把两者的液体、时间或地域事实合并。搜集层增加来源版本不等于对外推荐或普通电饭煲适配。

### 本批纪律

- 研究层允许扩大真实具名菜池，但每条数字、程序和器具只记录来源直接写明的范围；未给固定单一份数的条目保持 `fixed_batch=null`，不取平均数伪造合同。
- 营养结构如实记录：鲷饭与鱿鱼饭为碳水+蛋白，炊き込みご飯含碳水、蛋白和膳食纤维；这只是筛选信息，不等于营养追踪或厨房验证。
- 本批未修改运行时，不调用 DeepSeek，不部署 production；下一步仍是继续搜集和来源抽查，晋升与厨房验证另行排期。
