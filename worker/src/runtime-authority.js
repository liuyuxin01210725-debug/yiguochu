import { canonicalJson, sha256Hex } from './rice-meal-selector.js';
...
export function runtimeCandidateContractHash(entry = {}, variant = null, catalogVersion = null) {
  return sha256Hex(canonicalJson(candidateContractPayload(entry, variant, catalogVersion)));
}
...
export function validateRuntimeCandidateAuthority(candidate = {}, runtimeCatalogOrEntry = {}) {
  ...
  const expectedCatalogVersion = runtimeCatalogOrEntry?.runtime_catalog_version
    ...
    || null;
  if (typeof authority.recipe_id !== 'string' || authority.recipe_id !== entry.recipe_id) errors.push('runtime_candidate_authority recipe_id does not match runtime entry');
  ...
  if (typeof expectedCatalogVersion !== 'string' || authority.catalog_version !== expectedCatalogVersion) errors.push('runtime_candidate_authority catalog_version does not match runtime catalog');
  ...
  const variant = variantId == null ? null : (Array.isArray(entry.approved_variants) ? entry.approved_variants.find(item => item?.variant_id === variantId) || null : null);
  ...
  if (candidate.recipe_runtime_catalog_version != null && candidate.recipe_runtime_catalog_version !== authority.catalog_version) errors.push('candidate recipe_runtime_catalog_version disagrees with runtime_candidate_authority');
...
export function attachRuntimeCandidateAuthority(candidate, entry, variant = null, catalogVersion = null) {
  return { ...candidate, runtime_candidate_authority: buildRuntimeCandidateAuthority(entry, variant, catalogVersion) };
}
...
export function runtimeVariantCandidateContractHash(variant = {}, familyId = null, catalogVersion = null) {
  return sha256Hex(canonicalJson(variantCandidateContractPayload(variant, familyId, catalogVersion)));
}
...
export function validateRuntimeVariantCandidateAuthority(candidate = {}, variant = {}, catalogVersion = null, familyId = null) {
  ...
  } else if (authority.contract_hash !== expected.contract_hash) {
    errors.push('runtime_candidate_authority contract_hash does not match rice runtime variant');
  }
